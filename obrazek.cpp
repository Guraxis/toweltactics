...
void Obrazovka::umisti(int x, int y)
{
	this->x = x;
	this->y = y;
}

void Obrazovka::pohni(int x, int y)
{
	this->x += x;
	this->y += y;
}
...
void Obrazovka::putpixel(int x, int y, Uint32 pixel)
{
	...
	if(x<0||y<0||x>=obrazovka->w||y>=obrazovka->h) return;
...
void Obrazek::umisti(float x, float y)
{
	this->x = x;
	this->y = y;
}
...
void Obrazek::pohni(float x, float y)
{
	this->x += x;
	this->y += y;
}
...
void Obrazek::kresli()
{
	...
	drect.x = (int)round(x) - obrazovka->x;
	drect.y = (int)round(y) - obrazovka->y;
...
{
	if((o->x + o->w) >= this->x && o->x <= (this->x + this->w) && (o->y + o->h) >= this->y && o->y <= (this->y + this->h)) return 1;
	else return 0;
}
